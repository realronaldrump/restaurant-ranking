...
- Implemented: quick-log circle-member tagging plus a complete Add More sheet with dishes, photos, visit type, subratings, price, occasion, canonical circle people, reusable named guests, reorder intent, hazy memory, and the single narrative Memory field.
- Implemented: unrated visits remain in history without ranking influence.

## Ranking engine

- Implemented: 0–100 one-decimal scores, four fixed reaction anchors, Bayesian-style certainty, recency decay, established-place movement guardrail, dish prediction, comparison evidence, ties, anchor evidence, provisional state, category ranks, overall ranks, and device-independent circle averages (`Ranking/RankingEngine.swift`).
...
- Implemented: normalized Circle, Person, Brand, Location, Visit, Rating, Dish, DishEntry, Photo, Comparison, and WantEntry Core Data entities; person UUIDs provide canonical linkage across visits, ratings, rankings, history, and wish-list attribution.
...
- Implemented: device-local active-circle and current-person selection keeps identity out of shared records and prevents one member’s ranking from contaminating another’s.
- Implemented: shared pending-visit cards, independent overall/dish reactions, deterministic attendee rendering, and canonical person reconciliation for legacy member/guest splits or concurrent same-name additions from multiple devices.
...
- Runtime evidence: all 72 unit tests and the 3 targeted end-to-end circle/onboarding UI tests pass on iPhone 17 Pro (iOS 26.3.1 simulator).
- Visual evidence: the seeded sample log was reviewed at iPhone 17 Pro dimensions; masthead, meal CTA, ranking table, touch targets, tab bar, and scroll behavior render cleanly.
