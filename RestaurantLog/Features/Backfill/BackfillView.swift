...
struct BackfillView: View {
    ...
    private func confirmationCard(_ cluster: BackfillCluster) -> some View {
        ...
        .task(id: "\(index)-\(query)") {
            ...
            if value.isEmpty, let coordinate = cluster.coordinate {
                candidates = await locationService.searchNearby(around: coordinate, radius: 150)
            } else {
    ...
    private var confirmationProgress: some View {
        VStack(spacing: 8) {
            ...
            ProgressView(value: Double(index), total: Double(max(1, clusters.count)))
    ...
    private var searchField: some View {
        ...
        .accessibilityIdentifier("backfill-place-search")
    }
    ...
    private func loadSelected(_ items: [PhotosPickerItem]) async {
        ...
        let photos = await ImageSanitizer.processSelected(selected, fallbackDate: nil)
