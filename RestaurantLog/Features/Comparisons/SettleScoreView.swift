...
struct SettleScoreView: View {
    ...
    private var empty: some View {
        EmptyLogView(
            title: "All settled",
            message: "Your ranking is up to date. New or edited ratings can create more questions.",
    ...
    private func comparisonQuestion(_ question: ComparisonQuestion) -> some View {
        ScrollView {
            VStack(spacing: 22) {
                ...
                Button("Too Close to Call") { answer(.tie, question: question) }.buttonStyle(SecondaryButtonStyle())
    ...
    private func comparisonButton(_ location: RestaurantLocation, side: Side, question: ComparisonQuestion) -> some View {
        Button { answer(side == .a ? .a : .b, question: question) } label: {
            HStack(spacing: 14) { Image(systemName: location.category.symbol).font(.title2); VStack(alignment: .leading) { Text(location.name).font(BBTheme.display(23)); Text(location.category.shortTitle).font(.caption).foregroundStyle(BBTheme.paper.opacity(0.72)) }; Spacer(); if let score = store.score(for: location) { Text(score.displayScore).font(BBTheme.score(24)) } }
                ...
                .background(BBTheme.oxblood, in: RoundedRectangle(cornerRadius: BBTheme.Radius.card, style: .continuous))
    ...
    private func choice(_ location: RestaurantLocation, outcome: ComparisonOutcome, against other: RestaurantLocation) -> some View {
        Button { record(outcome, other) } label: {
            HStack {
                ...
                Text(location.name).font(BBTheme.display(23))
                Spacer()
                if let score = store.score(for: location) { Text(score.displayScore).font(BBTheme.score(22)) }
            }
            .padding(19)
            .foregroundStyle(BBTheme.paper)
            .background(BBTheme.oxblood, in: RoundedRectangle(cornerRadius: BBTheme.Radius.card, style: .continuous))
